                ...
                <table id="file_export" class="table w-100 table-striped table-bordered display text-nowrap">
                    ...
                    <tbody>
                        @foreach ($students as $index => $student)
                            @php
                                $grades = $student->grades->where('grd_semester_id', $semesterId);
                                $average = $grades->where('grd_subject_id', '!=', 1)->avg('grd_average');
                                $attendance = $grades->firstWhere('grd_subject_id', 1);
                            @endphp
                            <tr>
                                <td>{{ $index + 1 }}</td>
                                <td>{{ $student->user->name }}</td>
                                {{-- <td>{{ number_format($average, 2) ?? '-' }}</td>
                                <td>{{ $attendance->grd_sick ?? 0 }}</td>
                                <td>{{ $attendance->grd_permission ?? 0 }}</td>
                                <td>{{ $attendance->grd_absence ?? 0 }}</td> --}}
                                <td>
                                    <a href="{{ route('teacher.reports.show', [
                                            'studentId' => $student->std_id,
                                            'semester_id' => $semesterId,
                                            'class_id' => $selectedClass->cls_id,
                                        ]) }}" class="btn btn-sm btn-primary">Lihat</a>
                                          {{-- <a href="{{ route('teacher.report.print', [$student->std_id, $semesterId]) }}" target="_blank" class="btn btn-danger">
                                            PDF
                                        </a> --}}
                                </td>
                            </tr>
